class TokenLevelEval():
    ...
    def eval(parsed_text : str, gold_text : str) -> float:
        tokens_estratti = parsed_text.replace(",","").replace(".","").replace(":","").replace("|","").replace("---","").lower().split()
        tokens_gs = gold_text.replace(",","").replace(".","").replace(":","").replace("|","").replace("---","").lower().split()
        n_correct_words = 0
